...
//#endif
        ...
        [[maybe_unused]] static inline bool isdigit(const std::string &string){
            return std::count_if(string.begin(), string.end(), [](unsigned char c){ return std::isdigit(c); }) == (long)string.length();
        }
        /**
        ...
        struct digit_sort
        {
            inline bool operator() (const std::string& struct1, const std::string& struct2)
            {
                auto getFileName = [](const std::string &path) ->std::string {
                    ...
                    if((long)lastDot == path.front() && (long)lastRightSlash == path.front()+1) // input is ./100.cpp
                        return path.substr(lastRightSlash+1, path.length());
